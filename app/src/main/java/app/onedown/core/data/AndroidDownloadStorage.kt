...
import app.onedown.core.engine.DownloadStorage
import app.onedown.core.engine.FileDownloadStorage
import app.onedown.core.engine.FileNames
...
class AndroidDownloadStorage @Inject constructor(@ApplicationContext context: Context) : DownloadStorage {
        ...
        val uri = when (state.request.destinationKind) {
            ...
            "media" -> {
                ...
                resolver.insert(MediaStore.Downloads.EXTERNAL_CONTENT_URI, ContentValues().apply {
                    ...
                    put(MediaStore.MediaColumns.RELATIVE_PATH, Environment.DIRECTORY_DOWNLOADS + "/OneDown")
    ...
    private fun existingNames(state: DownloadState): Set<String> {
        ...
        val args = if (tree == null) arrayOf(Environment.DIRECTORY_DOWNLOADS + "/OneDown/") else null
