...
import app.onedown.core.data.AndroidDownloadStorage
import app.onedown.core.engine.DownloadOutput
...
class DirectDownloadWriter(context: Context, private val report: (String) -> Unit) {
        ...
        executor.execute {
            try {
                ...
                if (last) {
                    ...
                    main.post {
                        if (token == key) page?.evaluateJavascript("window[${JSONObject.quote("onedown_$key")}]?.();", null)
                    }
                }
            } catch (failure: Exception) { fail(key, failure) }
        }
    ...
    private fun removePageBridge() {
        pageToken?.let { key -> page?.evaluateJavascript("delete window[${JSONObject.quote("onedown_$key")}];", null) }
        page = null
        pageToken = null
    }
    ...
    private fun blobScript(key: String, url: String): String = """
        (async () => {
          const key = ${JSONObject.quote(key)}, name = ${JSONObject.quote("onedown_$key")};
          try {
            ...
            window[name] = async () => {
              try {
                ...
                if (!OneDownBlob.chunk(key, index++, String(blob.size), btoa(binary), last)) throw new Error('Rejected chunk');
                offset = end;
                if (last) delete window[name];
              } catch (_) { delete window[name]; OneDownBlob.failed(key); }
            };
            window[name]();
          } catch (_) { delete window[name]; OneDownBlob.failed(key); }
        })();
